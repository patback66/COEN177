...
PRIVATE struct proc *pick_random(struct proc *head)
{
	int i, n = 0;
	struct proc *rp;
	u64_t r;
	...
	/* Use low-order word of TSC as pseudorandom value. */
	i = r.lo % n;
	/*iterate through the current queue for the randomly chosen number of times*/
	for(rp = head; i--; rp = rp->p_nextready)
		;
...
#endif
	/*select a random process from the current queue*/
	rp = pick_random(rdy_head[q]);
